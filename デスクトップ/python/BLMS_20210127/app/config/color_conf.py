...
"orchid","palegoldenrod","palegreen","paleturquoise","palevioletred","papayawhip","peachpuff","peru","pink","plum","powderblue","purple","rebeccapurple","red",
...
#似た色を避けるため10個目からスタート
def COLOR(num):
    return COLORS[num*4 + 3]

#ジャンルで色分け
def GENRE(genre):
    for i,name in enumerate(config.genre_conf.GENRES):
        if genre == name:
            value = COLOR(i)
    return value
